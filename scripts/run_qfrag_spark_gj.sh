...
if [ $1 = "DEBUG" ] ; then
    $SPARK_HOME/bin/spark-submit --driver-memory $driverMemory --conf spark.driver.extraJavaOptions=-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=5005 spark.driver.maxResultSize=$maxResultSize --verbose --class io.arabesque.GJRunner $ARABESQUE_JAR -y $(basename $tempfile)
else
    $SPARK_HOME/bin/spark-submit --driver-memory $driverMemory --conf spark.driver.maxResultSize=$maxResultSize --verbose --class  io.arabesque.GJRunner $ARABESQUE_JAR -y $(basename $tempfile)
fi
